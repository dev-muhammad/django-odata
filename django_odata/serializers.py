...
class ODataSerializer(ODataSerializerMixin, FlexFieldsSerializerMixin, serializers.Serializer):
    """
    ...
    def _process_odata_params(self):
        ...
        if '$select' in odata_params:
            ...
            if not hasattr(self.context, 'request'):
                self.context['request'] = type('MockRequest', (), {
                ...
                if hasattr(self.context['request'], 'query_params'):
                    ...
                    if hasattr(self.context['request'].query_params, '_mutable'):
                        ...
                        self.context['request'].query_params['fields'] = ','.join(select_fields)
        ...
        if '$expand' in odata_params:
            ...
            if not hasattr(self.context, 'request'):
                ...
                if hasattr(self.context['request'], 'query_params'):
                    ...
                    if hasattr(self.context['request'].query_params, '_mutable'):
                        self.context['request'].query_params._mutable = True
                        self.context['request'].query_params['expand'] = ','.join(expand_fields)
                        self.context['request'].query_params._mutable = False
                    else:
                        # For regular dict objects in tests
                        self.context['request'].query_params['expand'] = ','.join(expand_fields)
        ...
        if hasattr(self.Meta, 'expandable_fields'):
            for field_name, config in self.Meta.expandable_fields.items():
                nav_props[field_name] = {
                    ...
                    'many': config[1].get('many', False) if isinstance(config, tuple) and len(config) > 1 else False,
...
class ODataListSerializer(serializers.ListSerializer):
    ...
    def to_representation(self, data):
        ...
        if request and '$format' in request.query_params:
            return {
                '@odata.context': self._get_context_url(),
                'value': items
            }
    ...
    def _get_context_url(self) -> str:
        ...
        """
        request = self.context.get('request')
        if request and hasattr(self.child, 'Meta') and hasattr(self.child.Meta, 'model'):
...
def create_odata_serializer(model_class, fields='__all__', expandable_fields=None, **kwargs):
    """
    ...
    serializer_class = type(serializer_name, (ODataModelSerializer,), {
        'Meta': Meta
    })
