...
class ODataModelViewSet(ODataMixin, viewsets.ModelViewSet):
    ...
    def get_odata_entity_set_name(self) -> str:
        ...
        """
        if hasattr(self.get_serializer_class(), 'Meta') and hasattr(self.get_serializer_class().Meta, 'model'):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            response.data['@odata.context'] = f"{base_url}$metadata#{entity_set}/$entity"
    ...
    @action(detail=True, methods=['get'], url_path=r'\$links/(?P<navigation_property>[\w-]+)')
    ...
    def get_navigation_property(self, request, navigation_property=None, pk=None):
        ...
        try:
            ...
            if related_obj is None:
                ...
                related_serializer_class = self._get_related_serializer_class(navigation_property)
                if related_serializer_class:
                    serializer = related_serializer_class(queryset, many=True, context=self.get_serializer_context())
                    return Response({
                ...
                related_serializer_class = self._get_related_serializer_class(navigation_property)
                if related_serializer_class:
                    serializer = related_serializer_class(related_obj, context=self.get_serializer_context())
                    data = serializer.data
                    data['@odata.context'] = f"{request.build_absolute_uri('/odata/')}$metadata#{navigation_property}/$entity"
                    return Response(data)
                else:
                    # Fallback to basic serialization
                    return Response({
                        '@odata.context': f"{request.build_absolute_uri('/odata/')}$metadata#{navigation_property}/$entity",
                        **{field.name: getattr(related_obj, field.name) for field in related_obj._meta.fields}
                    })
