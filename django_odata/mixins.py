...
class ODataSerializerMixin:
    ...
    def get_odata_context(self) -> Dict[str, Any]:
        ...
        context = {
            'odata_version': '4.0',
            'service_root': getattr(self.context.get('request'), 'build_absolute_uri', lambda x: x)('/odata/'),
        }
    ...
    def to_representation(self, instance):
        ...
        if request and hasattr(self, 'Meta') and hasattr(self.Meta, 'model'):
            ...
            include_context = query_params.get('$format') == 'json' or \
                            headers.get('Accept', headers.get('HTTP_ACCEPT', '')).startswith('application/json')
            
            if include_context and hasattr(instance, 'pk'):
                odata_context = self.get_odata_context()
                data['@odata.context'] = f"{odata_context['service_root']}$metadata#{odata_context['entity_set']}/$entity"
    ...
    def _process_odata_params_before_init(self, *args, **kwargs):
        ...
        if '$expand' in odata_params:
            ...
            expand_fields, nested_field_selections = self._parse_expand_expression(expand_value)
        ...
        for char in expand_value + ",":  # Add comma to process last field
            if char == "(" and not paren_depth:
                ...
                if current_field.strip():
                    field_name, nested_fields = self._process_expand_field(current_field.strip())
    ...
    def _process_expand_field(self, field):
        ...
        if inner_content.startswith("$select="):
            ...
            nested_fields = [f"{field_name}.{f.strip()}" for f in select_fields.split(',') if f.strip()]
...
class ODataMixin:
    ...
    def list(self, request, *args, **kwargs):
        ...
        odata_params = self.get_odata_query_params()
        include_count = '$count' in odata_params and odata_params['$count'].lower() == 'true'
        ...
        if hasattr(self, 'get_serializer_class'):
            ...
            if hasattr(serializer_class, 'Meta') and hasattr(serializer_class.Meta, 'model'):
                ...
                response_data['@odata.context'] = f"{request.build_absolute_uri('/odata/')}$metadata#{model_name}s"
