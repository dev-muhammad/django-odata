...
from django_odata.utils import (
    parse_odata_query, apply_odata_query_params, 
    build_odata_metadata, ODataQueryBuilder
)
...
class TestParseODataQuery(TestCase):
    ...
    def test_parse_basic_odata_params(self):
        ...
        query_params = QueryDict('$filter=name eq "test"&$orderby=value desc&$top=10&$skip=5')
    ...
    def test_parse_mixed_params(self):
        ...
        query_params = QueryDict('$filter=value gt 10&$select=name,value&$orderby=name asc')
        result = parse_odata_query(query_params)
...
class TestODataQueryBuilder(TestCase):
    ...
    def test_basic_query_building(self):
        ...
        result = builder.filter("name eq 'test'").order('value', desc=True).limit(10).build()
    ...
    def test_multiple_filters(self):
        ...
        expected = {
            '$filter': "(name eq 'test') and (value gt 5)"
        }
        self.assertEqual(result, expected)
