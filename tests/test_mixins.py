...
class TestODataMixin(TestCase):
    ...
    def test_get_odata_query_params(self):
        ...
        request = self.factory.get('/test/?$filter=name eq "test"&$top=10&$select=name,value')
    ...
    def test_list_method_response_format(self):
        ...
        viewset.get_serializer = lambda data, many=False: type('MockSerializer', (), {
            'data': [{'id': 1, 'name': 'test'}] if many else {'id': 1, 'name': 'test'}
        })()
