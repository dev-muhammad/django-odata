...
class AuthorSerializer(ODataModelSerializer):
    ...
    class Meta:
        ...
        expandable_fields = {
            "posts": (
                "tests.integration.test_enhanced_expand.BlogPostSerializer",
                {"many": True},
            ),
        }
...
class TestEnhancedExpandIntegration(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="author1", email="author1@example.com"
        )
        self.user2 = User.objects.create_user(
            username="author2", email="author2@example.com"
        )

        self.author1 = Author.objects.create(
            user=self.user1, bio="Author 1 Bio", website="https://author1.com"
        )
        self.author2 = Author.objects.create(
            user=self.user2, bio="Author 2 Bio", website="https://author2.com"
        )

        # Create categories
        self.cat_tech = Category.objects.create(
            name="Technology", description="Tech posts"
        )
        self.cat_science = Category.objects.create(
    ...
    def test_expand_with_filter_on_many_relation(self):
        ...
        request = self.factory.get(
            "/posts/?$expand=categories($filter=name eq 'Technology')"
        )
        viewset = BlogPostViewSet()
        viewset.request = request
        viewset.format_kwarg = None
    ...
    def test_nested_expand(self):
        ...
        request = self.factory.get("/posts/?$expand=author($expand=posts($top=1))")
        viewset = BlogPostViewSet()
        viewset.request = request
        viewset.format_kwarg = None
    ...
    def setUp(self):
        ...
        for i in range(10):
            ...
            author = Author.objects.create(
                user=user, bio=f"Author {i} Bio", website=f"https://author{i}.com"
            )
