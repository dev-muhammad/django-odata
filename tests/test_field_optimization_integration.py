...
from example.blog.models import BlogPost, Author, Category, Comment
...
class TestCompleteFieldOptimization(TestCase):
    ...
    def setUp(self):
        ...
        self.tech = Category.objects.create(
            name="Technology",
            description="Tech articles with long descriptions"
        )
        self.science = Category.objects.create(
            name="Science",
            description="Science articles with long descriptions"
        )
    ...
    def test_query_count_optimization(self):
        ...
        # With optimization
        request2 = self.factory.get('/posts/', {
            '$select': 'id,title',
            '$expand': 'categories($select=name)'
        })
...
class TestFieldOptimizationEdgeCases(TestCase):
    ...
    def test_nonexistent_field_in_select(self):
        ...
        request = self.factory.get('/posts/', {
            '$select': 'id,title,nonexistent_field',
            '$expand': 'author'
        })
        viewset = BlogPostViewSet()
        viewset.request = request
    ...
    def setUp(self):
        ...
        for i in range(10):
            ...
            author = Author.objects.create(
                user=user,
                bio=f"Bio for author {i} with lots of text " * 10
            )
