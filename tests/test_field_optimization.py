...
class TestFieldSelectionOptimization(TestCase):
    ...
    def setUp(self):
        ...
        self.author = Author.objects.create(user=self.user, bio="Test author bio")
    ...
    def test_foreign_keys_included_for_expand(self):
        ...
        request = self.factory.get(
            "/posts/", {"$select": "id,title", "$expand": "author"}
        )
        viewset = BlogPostViewSet()
        viewset.request = request
        viewset.format_kwarg = None
...
class TestBuildOnlyFieldsList(TestCase):
    ...
    def setUp(self):
        ...
        self.factory = APIRequestFactory()
        self.user = User.objects.create_user(
            username="testauthor", email="test@example.com"
        )
    ...
    def test_includes_foreign_key_for_expand(self):
        ...
        request = self.factory.get("/posts/", {"$select": "title", "$expand": "author"})
        viewset = BlogPostViewSet()
        viewset.request = request
        viewset.format_kwarg = None
    ...
    def test_handles_multiple_expansions(self):
        ...
        request = self.factory.get(
            "/posts/", {"$select": "title", "$expand": "author,categories"}
        )
        viewset = BlogPostViewSet()
        viewset.request = request
        viewset.format_kwarg = None
...
class TestFieldSelectionIntegration(TestCase):
    ...
    def setUp(self):
        ...
        self.author = Author.objects.create(
            user=self.user, bio="Bio for integration testing"
        )
